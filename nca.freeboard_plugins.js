...
(function () {
  ...
  var ncaGoogleMapWidget = function (settings) {
        var self = this;
        var currentSettings = settings;
        var map;
        var currentEntity=[];
		...
		for (icp = 0; icp < currentEntity.length; icp++) {

					var wasteKindOk = false;
					if (currentSettings.storedWasteKind == "all" || (currentEntity[icp].storedWasteKind == currentSettings.storedWasteKind)){ 
							wasteKindOk = true;
						};
					var addressLocalityOk = false;
					if (currentSettings.addressLocality == "all" || (currentEntity[icp]. addressLocality == currentSettings. addressLocality)){   
								addressLocalityOk = true;
							};
					var locationNcaOK = false;
						if (map && currentEntity[icp].lat && currentEntity[icp].lon){
								locationNcaOK = true;
						};
					
					if ( locationNcaOK == true && wasteKindOk == true && addressLocalityOk == true){
					
									var latLon = new google.maps.LatLng(currentEntity[icp].lat, currentEntity[icp].lon);
									var label = {};
									label.text = currentEntity[icp].name;
									label.color = "white";              

									var iconBase = 'http://freeboard.nicecotedazur.org/freeboard/img/';
									var icons = {
										green: {
											icon: iconBase + 'wastegreen.png'
										},
										yellow: {
											icon: iconBase + 'wasteyellow.png'
										},
										red: {
											icon: iconBase + 'wastered.png'
										}
								   };
									var icon = icons['green'].icon;
									if (parseFloat(currentEntity[icp].fillingLevel) < 0.6)
									{
										icon = icons['green'].icon;
									}
									else if (parseFloat(currentEntity[icp].fillingLevel) > 0.79) {
										icon = icons['red'].icon;
									}
									else {
									...
									var marker = new google.maps.Marker({
										...
										icon: icon,
										map: map,
										title: currentEntity[icp].name
									});

									var dateLastEmptying = new Date(currentEntity[icp].dateLastEmptying);
									var dateLastEmptyingformat = ("0" + dateLastEmptying.getDate()).slice(-2) + '/' + ("0" + (dateLastEmptying.getMonth() + 1)).slice(-2) + '/' + dateLastEmptying.getFullYear().toString().substr(0, 4) + '\t' + ("0" + dateLastEmptying.getHours()).slice(-2) + ':' + ("0" + dateLastEmptying.getMinutes()).slice(-2) + ':' + ("0" + dateLastEmptying.getSeconds()).slice(-2);
									var contentString = '<div id="content">'+
									'<div id="head">'+
									'</div>'+
									'<h43 id="firstHeading" class="firstHeading">' + currentEntity[icp].name + '</h4>'+
									'<div id="bodyContent">'+
									'<p><b>Taux de remplissage : </b>' + currentEntity[icp].fillingLevel + '</br>' +
									'<b>Type : </b>' + currentEntity[icp].storedWasteKind + '</br>' +
									'<b>Dernier ramasassage : </b>' + dateLastEmptyingformat + '</br>' +
									
									'<b>Commune : </b>' + currentEntity[icp].address + '</br>' + '</p>' +
			...
			} //fermeture de  boucle for
	 } //fermeture de la function updatePosition
        ...
        this.render = function (element) {
            function initializeMap() {
                var mapOptions = {
                    zoom: 11,
                    center: new google.maps.LatLng(43.70, 7.25),
				...
				markers = new Array(1);
                markers[0] = new google.maps.Marker({ map: map });
        ...
        this.onCalculatedValueChanged = function (settingName, newValue) {  
            if (settingName == "datasource") {
                var contexts = $.map(newValue, function (value, index) {
                    return [value];
                });
			 
                markers = new Array(contexts.length);
				currentEntity = [];
				
                for (ic = 0; ic < contexts.length; ic++) {
                    markers[ic] = new google.maps.Marker({ map: map });
                    coordinates = contexts[ic]["location"]["coordinates"];
                    entity = {};
                    entity.name = contexts[ic]["name"];;
                    entity.fillingLevel = parseFloat(contexts[ic]["fillingLevel"]);
                    entity.storedWasteKind = contexts[ic]["storedWasteKind"];
					entity.address = contexts[ic]["address"]["addressLocality"] + ", " + contexts[ic]["address"]["addressCountry"];
					entity.addressLocality = contexts[ic]["address"]["addressLocality"];
					entity.dateLastEmptying = contexts[ic]["dateLastEmptying"];
                    entity.lat = coordinates[1];
                    entity.lon = coordinates[0];
                    currentEntity[ic] = entity;
                }
    ...
    freeboard.loadWidgetPlugin({
        type_name: "nca_google_map",
        display_name: "NCA Google Map",
        fill_size: true,
        settings: [
		//.....................
            {
		...
		//.................................
			  ...
			  newInstance: function (settings, newInstanceCallback) {
            newInstanceCallback(new ncaGoogleMapWidget(settings));
        }
});
